...
do 
	-- 
	writefile("smallest_pixel.ttf", game:HttpGet("https://raw.githubusercontent.com/NymeraAnHomie/Fonts/main/ProggyTiny.ttf"))
	-- 
...
do -- Library
		...
		function Library:NewPicker(name, default, defaultalpha, parent, count, flag, callback)
			...
			Library:Connection(game:GetService("UserInputService").InputChanged, function(input)
				if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
					if slidingalpha or slidinghue or slidingsaturation then
						update()
					end
				end
			end)
	...
	do
		...
		function Library:Window(Options, A, B)
			...
			Library:Connection(UserInputService.InputChanged, function(input)
				if Window.Dragging[1] and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
					local location = UserInputService:GetMouseLocation()
		...
		function Sections:Slider(Properties)
			...
			Library:Connection(Frame.InputEnded, function(input)
				if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and input == SlidingInput then
					Sliding = false
					SlidingInput = nil
				end
			end)
		...
		function Pages:PlayerList(Properties)
			...
			List.Size = UDim2.new(1, -10, 0, 275)
...
do
    ...
    function Library:updateNotifsPositions(position)
        for i, v in pairs(Library.Notifs) do
            ...
            game:GetService("TweenService"):Create(v.Container, TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
    ...
    function Library:Notification(message, duration, color, flash)
        ...
        task.spawn(function()
            ...
            for i, v in next, NewInd:GetDescendants() do
                if v:IsA("Frame") then
                    game:GetService("TweenService"):Create(v, TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
            ...
            game:GetService("TweenService"):Create(ActualInd, TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
                AnchorPoint = Vector2.new(0, 0)
            }):Play()
            game:GetService("TweenService"):Create(TextLabel, TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
                TextTransparency = 0
            }):Play()
            task.wait(duration)
            for i, v in next, NewInd:GetDescendants() do
